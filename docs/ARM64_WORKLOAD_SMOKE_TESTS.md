...
```text
27 passing / 2 unsupported / 0 failed
report: /workspace/tmp/ish-arm64-cli-corner-smoke-20260516-223418.md
```
...
- Docker coverage now includes the CLI, `dockerd --version`, a bounded daemon startup attempt, and `docker run --rm hello-world`. In the Alpine fakefs the Docker packages install and the daemon binary executes, but daemon startup is reported as `UNSUPPORTED` when kernel/container primitives such as namespaces/cgroups/mount behavior are not available, and `hello-world` is reported as `UNSUPPORTED` when no daemon is available rather than pretending a container ran.
