...
| CLI corner-case smoke | Passing, Alpine 26/26 | Optional/diagnostic harness for CLI/TUI/network/container tools likely to expose terminal, `/proc`, DNS, ptrace, netlink, Unix-socket, and package-manager edge cases. Covers `nushell`/`xonsh` availability, `htop` and `btop` inside `tmux`, `tcpdump`, `traceroute`, `iproute2`, `bind-tools`, HTTPS `git` diagnostics including `rcarmo/go-gte` clone, Docker CLI `hello-world`, `strace`, `lsof`, `file`, `jq`, and Linuxbrew availability. | `/workspace/tmp/ish-arm64-cli-corner-smoke-20260516-153555.md` |
...
```text
26 / 26 passing
report: /workspace/tmp/ish-arm64-cli-corner-smoke-20260516-153555.md
```
...
- Raw libcurl/c-ares DNS can fail for HTTPS tools even when `getent`/`nslookup` resolve the same host. The harness keeps a direct `git ls-remote` diagnostic row and separate `/etc/hosts` workaround rows, including a real shallow clone of `https://github.com/rcarmo/go-gte.git`, so DNS regressions remain visible until the resolver path is fixed.
