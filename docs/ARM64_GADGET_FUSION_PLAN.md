...
Phase 4 dispatch-adjacent trace-record dry-run tranche:

- Added a hot-trace-gated dispatch-adjacent sidecar lookup that runs after normal block lookup with `frame->last_block` and the current whole-block entry, validates any matching `arm64_trace_record`, and then always falls back to normal execution. This is still non-executing: no trace body, no generated trace code, no guarded exits, no invalidation epoch changes, and no trace/interior pointers are written into `jump_ip`, ret-cache, `LOCAL_last_block`, `fiber_ret_chain`, or `inline_chain` paths.
- Extended `ARM64_BLOCK_HOT_STATS` with `hot_trace_record_dispatch_*` counters for probes, lock-busy skips, hits, misses, guard pass/fail, and forced fallbacks. The fixture now asserts stats-only runs keep dispatch probes at zero and stats+hot-trace runs report dispatch hits with `hot_trace_record_dispatch_hits == hot_trace_record_dispatch_fallbacks`, proving the dry-run path does not take over execution. Report `/workspace/tmp/ish-arm64-hot-trace-record-smoke-20260517-152231.md` was **8 / 8 passing** with `hot_trace_record_dispatch_probes=3173853`, `hot_trace_record_dispatch_hits=8575`, and `hot_trace_record_dispatch_fallbacks=8575` in the prior enabled validation row; the final fixture rerun kept the same invariants while also asserting guard passes cover all created records.
- Validation after dispatch dry-run counters: default Node/Bun `/workspace/tmp/ish-arm64-node-bun-perf-20260517-150517.md` and stats+hot-trace Node/Bun `/workspace/tmp/ish-arm64-node-bun-perf-20260517-150600.md` were both **10 / 10 passing**; default Alpine runtime coverage `/workspace/tmp/ish-arm64-runtime-coverage-20260517-150644.md` was **83 / 83 passing**.
