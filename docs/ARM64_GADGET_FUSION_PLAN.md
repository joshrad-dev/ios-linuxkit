...
- Audit follow-up: added default-off `ISH_ARM64_EAGER_PRECHAIN_INCOMING=1` as a second-stage experiment under `ISH_ARM64_EAGER_PRECHAIN=1`. It scans only a small bounded set of already-compiled same-page predecessor blocks and patches still-fake slots that target the newly inserted block. Outgoing and incoming eager patches now have split counters: `prechain_outgoing_attempts`, `prechain_outgoing_patches`, `prechain_incoming_attempts`, and `prechain_incoming_patches`. Dynamic ARM64 chain patching now validates the bit-63 fake-IP tag and full 48-bit target before replacing a slot with `block->code`.
- Validation reports:
